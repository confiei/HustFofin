...
export default function PaginaLimparDmAmigos() {
  ...
  useWSEvent<WSToolCompleted>("tool:completed", (data) => {
    ...
    if (notificationsRef.current && notifiedTaskRef.current !== taskId) {
      ...
      notify("HustFofin", `Limpeza de DMs concluída! ${progress} mensagens deletadas`)
    }
  })
  ...
  useWSEvent<WSToolError>("tool:error", (data) => {
    ...
    if (notificationsRef.current && notifiedTaskRef.current !== taskId) {
      notifiedTaskRef.current = taskId
      notify("HustFofin", `Erro na limpeza: ${data.error || "Erro desconhecido"}`)
    }
  })
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setElapsedSeconds(0)
      if (notificationsRef.current) notify("HustFofin", `Limpeza de DMs iniciada (${data.totalAmigos} amigos)`)
    } catch (err: unknown) {
