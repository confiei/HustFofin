...
export default function PaginaLimparDm() {
  ...
  useWSEvent<WSToolCompleted>("tool:completed", (data) => {
    ...
    if (notificationsRef.current && notifiedTaskRef.current !== taskId) {
      ...
      notify("HustFofin", `Limpeza concluída! ${progress} mensagens deletadas de ${username}`)
    }
    api.getAnalytics().then((res) => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setElapsedSeconds(0)
      if (notificationsRef.current) notify("HustFofin", `Limpeza iniciada com ${data.username}`)
    } catch (err: unknown) {
