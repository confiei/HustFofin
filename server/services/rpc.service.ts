...
const DEFAULT_IMAGE = 'https://imgur.com/lc9XmYH'
...
function fetchAppIcon(appId: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    const req = https.get(url, { headers: { 'User-Agent': 'HustFofin-RPC/1.0' }, timeout: 3000 }, (res) => {
      let data = ''
...
export function fetchAppInfo(appId: string): Promise<{ name: string; icon: string | null }> {
  return new Promise((resolve) => {
    const url = `https://discord.com/api/v10/applications/${appId}/rpc`
    const req = https.get(url, { headers: { 'User-Agent': 'HustFofin-RPC/1.0' }, timeout: 5000 }, (res) => {
      let data = ''
      res.on('data', (chunk) => (data += chunk))
      res.on('end', () => {
        try {
