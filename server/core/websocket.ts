...
class WebSocketManager {
  ...
  init(server: Server) {
    const WSServer = (ws as any).WebSocketServer || (ws as any).default?.WebSocketServer || (ws as any).Server || ws
    ...
    this.wss.on('connection', (socket: any) => this.handleConnection(socket))
    this.startHeartbeat()
  ...
  private subscribeToLogs() {
    this.logUnsubscribe = logger.subscribe((entry) => {
      ...
      for (const [clientId, client] of this.clients) {
        // readyState 1 significa WebSocket.OPEN
        if (client.subscribedToLogs && client.ws && client.ws.readyState === 1) {
